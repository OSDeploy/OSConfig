...
if ($ProductName -like "*Windows 7*") {
    if (Test-Path "$env:ProgramData\OSConfig\Theme\Win7.theme") {
		Write-Host "Applying Windows 7 Custom Theme" -ForegroundColor Cyan
        Copy-Item "$env:ProgramData\OSConfig\Theme\Win7.theme" -Destination "$env:ProgramData\OSConfig\Theme\Custom.theme" -Force
		Write-Host ""
    }
}
if ($ProductName -like "*Windows 10*") {
    if (Test-Path "$env:ProgramData\OSConfig\Theme\Win10.theme") {
		Write-Host "Applying Windows 10 Custom Theme" -ForegroundColor Cyan
        Copy-Item "$env:ProgramData\OSConfig\Theme\Win10.theme" -Destination "$env:ProgramData\OSConfig\Theme\Custom.theme" -Force
		Write-Host ""
    }
}
if ($ProductName -like "*Server 2016*") {
    if (Test-Path "$env:ProgramData\OSConfig\Theme\Server2016.theme") {
		Write-Host "Applying Windows Server 2016 Custom Theme" -ForegroundColor Cyan
        Copy-Item "$env:ProgramData\OSConfig\Theme\Server2016.theme" -Destination "$env:ProgramData\OSConfig\Theme\Custom.theme" -Force
		Write-Host ""
    }
}
#======================================================================================
